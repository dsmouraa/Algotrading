...
for TICKER in TICKERS:
    ...
    df = np.round(yf.download(TICKER, start=DATA_INICIO, end=DATA_FINAL)[-248:], 2)
    ...
    for param in PARAM:
        ...
        # Encontra a entrada
        df['Entrada'] = np.round((df['Open'] * parametro)/100 + df['Open'], 2)
        
        # Resultado da entrada menos o valor de fechamento sem condição
        df['Resultado'] = np.round((df['Close'].values - df['Entrada'].values)*100, 2).tolist()
        
        # Executa a condição para verificar se a entrada no dia foi executada ou não
        mask = df['Low'].values <= df['Entrada'].values
...
df_final.to_csv(f'resultados_CA.csv')
